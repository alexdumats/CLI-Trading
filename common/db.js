...
export function createPgPool() {
  ...
  if (process.env.POSTGRES_PASSWORD_FILE && !password) {
    try { password = require('fs').readFileSync(process.env.POSTGRES_PASSWORD_FILE, 'utf8').trim(); } catch {}
  }
  const config = {
...
export async function insertAudit(pool, { type, severity = 'info', payload = {}, requestId = null, traceId = null }) {
...
export async function upsertPnl(pool, { date, startEquity, realized, percent, dailyTargetPct, halted }) {
